    ...
    public class CustomDownloader
    {
        ...
        public async Task StartDownloadAsync()
        {
            ...
            try
            {
                ...
                {
                    var (totalBytes, supportsRanges) = await ProbeServerAsync(DownloadUrl);
                ...
                var progressTask = Task.Run(async () =>
                {
                    ...
                    while (_state == DownloadState.Downloading || _state == DownloadState.Paused)
                    {
                        ...
                        var speed = dt > 0 ? (delta / (1024.0 * 1024.0)) / dt : 0;
                        var pct = _manifest.TotalBytes > 0 ? (totalDownloaded * 100.0 / _manifest.TotalBytes) : 0;
                        var eta = speed > 0 && _manifest.TotalBytes > 0
                            ? TimeSpan.FromSeconds((_manifest.TotalBytes - totalDownloaded) / (speed * 1024 * 1024))
                            : TimeSpan.Zero;
                        ...
                        if (_state == DownloadState.Cancelled) break;
                    }
                });
        ...
        private async Task SingleThreadDownloadAsync()
        {
            ...
            while ((bytesRead = await contentStream.ReadAsync(buffer, 0, buffer.Length, _cts.Token)) > 0)
            {
                ...
                if (elapsed - lastTime >= 0.3)
                {
                    ...
                    var eta = speed > 0 && totalBytes > 0
                        ? TimeSpan.FromSeconds((totalBytes - totalRead) / (speed * 1024 * 1024))
                        : TimeSpan.Zero;
                    ProgressChanged?.Invoke(this, new DownloadProgressStats
                    {
        ...
        private async Task<(long totalBytes, bool supportsRanges)> ProbeServerAsync(string url)
        {
            using var client = CreateHttpClient();
            ...
            try
            {
                ...
                Logger.Log($"[Downloader] Probe: {totalBytes} bytes, ranges={acceptRanges}");

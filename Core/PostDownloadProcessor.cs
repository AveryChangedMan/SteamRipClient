    ...
    public static class PostDownloadProcessor
    {
            ...
            Func<long, long, Task<bool>>? confirmSpace = null)
        {
            ...
            try {
                ...
                {
                    Logger.Log($"[PostDownload] ⚠️ SPACE WARNING: {gameTitle} needs ~{requiredSpace / 1024 / 1024}MB, but only {freeSpace / 1024 / 1024}MB is free.");
                    if (confirmSpace != null)
                    {
                        bool proceed = await confirmSpace(freeSpace, requiredSpace);
                        if (!proceed)
                        {
                            onStatus?.Invoke("❌ Installation cancelled: Insufficient space.");
                            return null;
                        }
                    }
                }
            ...
            bool extracted = await ArchiveExtractor.ExtractAsync(
                ...
                onStatus: onStatus,
                deleteAfter: GlobalSettings.AutoDeleteArchive
            );
            if (!extracted)
            {
            ...
            var extractedGameDir = FindExtractedGameDir(gameExtractionDir, archivePath)
                                   ?? gameExtractionDir;
            Logger.Log($"[PostDownload] Extracted game content dir: {extractedGameDir}");
            onStatus?.Invoke($"📂 Found: {Path.GetFileName(extractedGameDir)}");
            onStatus?.Invoke("🖼 Copying cover image...");
            string? localImagePath = null;
            if (!string.IsNullOrEmpty(imageUrl))
            {
                await ScannerEngine.DownloadGameImageAsync(imageUrl, extractedGameDir);
                var expectedPath = Path.Combine(extractedGameDir, "folder.jpg");
            ...
            if (!string.IsNullOrEmpty(steamRipPageUrl))
                GlobalSettings.GamePageLinks[extractedGameDir] = steamRipPageUrl;

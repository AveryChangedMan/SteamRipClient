...
namespace SteamRipApp
{
    public sealed partial class LibraryPage : Page
    {
            ...
            try {
                ...
                foreach (var game in results)
                {
                    if (GlobalSettings.GamePageLinks.TryGetValue(game.RootPath, out var savedUrl) && !string.IsNullOrEmpty(savedUrl))
                        game.Url = savedUrl;
                    var metadata = GlobalSettings.Library.FirstOrDefault(m => m.LocalPath == game.RootPath);
                    if (metadata != null)
                    {
        ...
        private void PollRedistStatus()
        {
            ...
            if (anyChanged)
            {
                ...
                if (!_activeRedistGame.HasMissingRedists)
                {
                    ...
                    RedistPanelClose_Click(null, null);
                }
            }
        }
        ...
        {
            string exeName = Path.GetFileNameWithoutExtension(gf.ExecutablePath);
            var dialog = new ContentDialog
            {
                Title = "Stop Process?",

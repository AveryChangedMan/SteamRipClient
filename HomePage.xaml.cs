    ...
    public sealed partial class HomePage : Page
    {
            ...
            try {
                ...
                if (bzFound && gfFound)
                {
                    ...
                    var selectDialog = new ContentDialog {
                        ...
                        Content = "Both Gofile and Buzzheavier are available. Which would you prefer?",
                        PrimaryButtonText = "Buzzheavier (Fast)",
        ...
        private async Task StartBuzzheavierDownload(SearchResult item)
        {
            ...
            try {
                ...
                var directUrl = await InterceptDownloadUrlAsync(bzzhrUrl);
                if (string.IsNullOrEmpty(directUrl))
                    directUrl = await SteamRipScraper.ExtractBuzzheavierDirectUrlAsync(bzzhrUrl);
        ...
        private async Task StartGoFileDownload(SearchResult item)
        {
            try {
                ...
                {
                    Logger.Log("[QuickDownload-GoFile] Direct links not pre-resolved, fetching now...");
                    if (!string.IsNullOrEmpty(item.GoFileUrl))
                        directLinks = await GoFileClient.GetDirectLinksAsync(item.GoFileUrl);
                }
        ...
        private async Task StartDownloadWithMetadata(SearchResult item, string destPath, string directUrl, string pageUrl, string sourceName, bool isGoFile = false)
        {
            ...
            downloader.DownloadCompleted += async (s, e) =>
            {
                ...
                this.DispatcherQueue.TryEnqueue(() => {
                    ...
                    {
                        metadata.Phase = "Done";
                        metadata.Status = $"✅ '{item.Title}' installed!";
                    ...
                    GlobalSettings.ActiveDownloads.Remove(metadata);
                    GlobalSettings.Save();
